...
public class Job extends DatabaseRow {
	public static final int PRIVILEGE_NONE = 0; // 0000000
	...
	public static final int PRIVILEGE_ALL = PRIVILEGE_ADMIN | PRIVILEGE_COOK
			| PRIVILEGE_SALESMAN | PRIVILEGE_DRIVER | PRIVILEGE_NUTRITIOUS
			| PRIVILEGE_RESOURCES | PRIVILEGE_STATISTICS; // 111111
	 ...
	 * @throws SQLException
	 */
	public static Job createDefault(DatabaseManager manager)
			throws SQLException {
		String sql = "INSERT INTO Job (name, yearly_salary, hourly_salary, percent_sales, user_privileges)"
				+ "VALUES ('', 0, 0, 0, 0)";
		Job ret = null;
	...
	public void fetch() throws SQLException {
		super.fetch();
		String sql = "SELECT name, yearly_salary, hourly_salary, percent_sales, user_privileges FROM Job WHERE id = "
				+ id;
	...
	public void commit() throws SQLException {
		...
		sql += ", user_privileges = " + privileges;
		sql += " WHERE id = " + id;
		try (PreparedStatement ps = getManager().prepareStatement(sql)) {
			ps.executeUpdate();
		}
	 ...
	 * 
	 * @param percentSales
	 *            The new percentage earned directly of sales
	 */
	...
	/**
	 * Give the selected employee one or more privileges from
	 * Employee.PRIVILEGE_*
	 * 
	 * @param privileges
	 */
	...
	public boolean hasPrivileges(int privileges) {
		assert (privileges >= 0);
		super.tryFetch();
		return (this.privileges & privileges) == privileges;
	}
	...
	private String printPrivileges() {
		...
		if ((privileges & PRIVILEGE_COOK) != 0) {
			printPriv += " has cook privileges, ";
		}
		if ((privileges & PRIVILEGE_SALESMAN) != 0) {
			printPriv += " has salesman privileges, ";
		}
		if ((privileges & PRIVILEGE_DRIVER) != 0) {
			printPriv += " has driver privileges, ";
		}
		if ((privileges & PRIVILEGE_NUTRITIOUS) != 0) {
			printPriv += " has nutritious privileges, ";
		}
		if ((privileges & PRIVILEGE_RESOURCES) != 0) {
	...
	public String toString() {
		...
		return "Job[name = '" + name + "'; yearlySalary = " + yearlySalary
				...
				+ percentSales + "; privileges = " + printPrivileges() + "]";
	}
}
